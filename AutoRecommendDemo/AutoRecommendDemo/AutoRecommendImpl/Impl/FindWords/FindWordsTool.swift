...
class FindWordsNode {
    /// 该节点如果是拼音的末尾节点，则存放该拼音对应的同音字，比如"zhong"中"g".words = ["重", "中"...]
    private var words: [String] = []
    /// 该节点对应的字母，小写存放
    private let value: String
    /// 包含的所有子节点，key为字母
    private var subNodes: [String: FindWordsNode] = [:]

    init(value: String) {
        self.value = value
    }

    func insert(pinYin: String, word: String) {
        // 如果后续没有字符需要继续插入，说明当前节点是一个完整节点，把汉字放入此节点
        guard !pinYin.isEmpty, let firstChar = pinYin.first?.lowercased() else { self.words.append(word); return }

        // 判断是否存在子节点，没有就创建
        self.subNodes[String(firstChar)] = self.subNodes[String(firstChar)] ?? FindWordsNode(value: String(firstChar))

        // 继续对对应子节点进行插入
        self.subNodes[String(firstChar)]?.insert(pinYin: String(pinYin.suffix(pinYin.count - 1)), word: word)
    }

    func find(pinYin: String) -> [String] {
        // 如果后续没有字符需要继续查找，说明当前节点是找到的结果
        guard !pinYin.isEmpty, let firstChar = pinYin.first?.lowercased() else { return self.words }

        guard let node = self.subNodes[String(firstChar)] else { return [] }
