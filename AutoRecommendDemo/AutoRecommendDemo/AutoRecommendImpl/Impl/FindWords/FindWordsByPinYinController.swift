//
//  FindWordsByPinYinController.swift
//  AutoRecommendDemo
//
...
class FindWordsByPinYinController: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    private var dataSource: [String] = []

    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = "通过拼音找到同音汉字"
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        self.dataSource = FindWordsTool.shared.findWords(pinYin: (textField.text ?? "").lowercased())
        if self.dataSource.isEmpty { self.dataSource = ["未找到该拼音对应的汉字"] }
